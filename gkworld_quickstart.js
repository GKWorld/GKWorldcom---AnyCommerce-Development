...
//				app.u.dump('BEGIN app.ext.convertSessionToOrder.formats.shipMethodsAsTable');
	...
	//app.u.dump(' -> id = '+id+' and ship.selected_id = '+app.data.cartItemsList.cart['ship.selected_id']);
						...
						if(isSelectedMethod)
							o += " checked='checked' "
						o += "/><label for='ship-selected_id_"+safeid+"'>"+shipName+": <\/label><\/td><td>"+app.u.formatMoney(data.value[i].amount,'$','',false)+"<\/td>"
						if(app.data.cartItemsList.cart['ship.country'] == "US" || !app.u.isSet(app.data.cartItemsList.cart['ship.country']))	{
							o += "<a href='#' onClick=\"$('#shipMessageDomestic').dialog({width:'60%','modal':'true',autoOpen:false}).dialog('open'); return false; \"> ? <\/a>";
							}
						o += "<td><\/td><\/tr>";
...
//					$img.remove(); //get rid of original img instance.
			...
			handleToolTip : function(container)	{
				if(container)	{
					app.u.dump("BEGIN myRIA.u.handleToolTip.");
					...
					app.u.dump("WARNING! handleToolTip required param not set: container");
					}
				},
